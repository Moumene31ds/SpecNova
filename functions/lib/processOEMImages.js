...
exports.onOemMediaJobCreated = void 0;
exports.processOEMImages = processOEMImages;
...
const COLLECTION = config_1.COLLECTIONS.oemMediaJobs;
const QcResultSchema = genkit_1.z.object({
    isCorrectDevice: genkit_1.z.boolean(),
    confidence: genkit_1.z.number().min(0).max(1),
    reasons: genkit_1.z.array(genkit_1.z.string()).default([]),
    suggestedColor: genkit_1.z.string().nullish(),
    transparentReady: genkit_1.z.boolean().default(false),
});
// ---------------------------------------------------------------------------
// Core pipeline
// ---------------------------------------------------------------------------
/** Full media pipeline for one job. Returns the terminal job state. */
async function processOEMImages(job) {
    ...
    const qc = {
        ...
        reviewedAt: firestore_2.Timestamp.now(),
    };
    if (!qc.passed) {
        console.info(`[oem-media] rejected ${job.sourceUrl} for ${device} ` +
    ...
    const cdn = await buildAndUpload(bucket, job.slug, source, assetBase, metadata.width, qcResult.transparentReady);
    // ---- 5. Backfill the device document ---------------------------------
    await backfillDevice(job, cdn, qc, metadata.width, metadata.height);
    const elapsed = Date.now() - started;
    console.info(`[oem-media] processed ${job.sourceUrl} -> ${job.slug} in ${elapsed}ms`);
...
async function runQc(job, source) {
...
async function buildAndUpload(bucket, slug, source, assetBase, sourceWidth, transparentReady) {
    const orientation = await (0, sharp_1.default)(source).metadata();
    ...
    const lqip = await (0, sharp_1.default)(source)
    ...
    if (transparentReady) {
        try {
            ...
            console.warn(`[oem-media] background removal skipped for ${assetBase}:`, err);
        }
    }
    return {
        original,
...
async function encodeVariant(source, width, format) {
    const pipeline = (0, sharp_1.default)(source)
        .rotate()
...
async function removeBackground(source) {
    const { data, info } = await (0, sharp_1.default)(source)
        .rotate()
        .removeAlpha()
        .raw()
    ...
    for (let p = 0, o = 0; p < px.length; p += channels, o += 4) {
        const d = Math.max(Math.abs(px[p] - key[0]), Math.abs(px[p + 1] - key[1]), Math.abs(px[p + 2] - key[2]));
        let alpha = 255;
        if (d < LO)
            alpha = 0;
        else if (d < HI)
            alpha = Math.round(((d - LO) / (HI - LO)) * 255);
        out[o] = px[p];
        out[o + 1] = px[p + 1];
        out[o + 2] = px[p + 2];
        out[o + 3] = alpha;
    }
    return (0, sharp_1.default)(out, { raw: { width, height, channels: 4 } })
        .png({ compressionLevel: 9 })
        .toBuffer();
}
async function upload(bucketName, objectPath, data, contentType, publicRead) {
    const file = (0, storage_1.getStorage)().bucket(bucketName).file(objectPath);
...
async function backfillDevice(job, cdn, qc, width, height) {
    const deviceRef = config_1.db.collection(config_1.COLLECTIONS.devices).doc(job.slug);
    const jobEntry = {
        id: job.id,
        ...
        createdAt: job.createdAt ?? firestore_2.Timestamp.now(),
        updatedAt: firestore_2.Timestamp.now(),
    };
    const patch = {
        "media.assetJobs": firestore_2.FieldValue.arrayUnion(jobEntry),
        [`media.cdn.${job.id}`]: cdn,
        "media.qcModel": "gemini-1.5-flash",
        "media.lastQcAt": firestore_2.Timestamp.now(),
    };
    if (qc.suggestedColor) {
        patch["media.colorVariants"] = firestore_2.FieldValue.arrayUnion({
    ...
    if (job.kind === "official") {
        patch["media.gallery"] = firestore_2.FieldValue.arrayUnion(cdn.original);
    }
 ...
 */
exports.onOemMediaJobCreated = (0, firestore_1.onDocumentCreated)({
...
//# sourceMappingURL=processOEMImages.js.map