...
import { SpecScraper } from "./scrapers/crawlee-scraper";
import { normalizeScrape, composeContent, type NormalizedSpec } from "./scrapers/normalize";
 ...
 */
export async function runIngestionPipeline(query: string): Promise<PipelineResult> {
  const startedAt = Date.now();

  const raw = await new SpecScraper().scrape(query);
  const spec: NormalizedSpec = await normalizeScrape(raw);
  const content = composeContent(spec);
  ...
  const deviceRef = db.collection(COLLECTIONS.devices).doc(slug);
  ...
  await deviceRef.set({
    ...
    announcedAt: spec.announcedAt ? Timestamp.fromDate(new Date(spec.announcedAt)) : null,
    releaseAt: spec.releaseAt ? Timestamp.fromDate(new Date(spec.releaseAt)) : null,
    ...
    specs: {
      ...
      sensors: [],
      extras: {
        fingerprint: spec.extras.fingerprint as "under-display" | "side" | "rear" | "none" | undefined,
        faceUnlock: false,
  ...
  console.info(
    `[pipeline] ingested "${brand} ${name}" (${slug}) in ${elapsed}ms`,
  );
...
function stripBrand(query: string, brand: string): string {
  return query
