...
import { autoFillDevice } from "@/actions/admin/autoFillDevice";
...
// ---------------------------------------------------------------------------
// Blank draft (create without AI)
// ---------------------------------------------------------------------------

function blankDraft(): AiExtractedDevice {
  return {
    brand: "",
    name: "",
    modelNumbers: [],
    ...
    specs: {
      ...
      cameras: { rear: [], front: [], features: [], videoCapabilities: [] },
      audio: { speakers: [], headphoneJack: false, codecs: [], microphone: null },
      battery: {
        capacityMah: null,
        type: null,
        chargingWatts: null,
        chargingTimeMin: null,
        wirelessWatts: null,
        reverseWirelessWatts: null,
        enduranceHours: null,
    ...
    pricing: { msrp: null, currentPrice: null, currency: "USD", region: null },
    software: { osUpdateYears: null, securityUpdateYears: null, aiPlatform: null },
    variants: [],
...
export default function NewDevicePage() {
  ...
  const [draft, setDraft] = useState<AiExtractedDevice | null>(null);
  const [raw, setRaw] = useState<string | null>(null);
  const [saving, setSaving] = useState<"draft" | "publish" | null>(null);
  const [saved, setSaved] = useState<{ id: string; slug: string; published: boolean } | null>(null);
  ...
  async function onAutofill() {
    ...
    const res = await autoFillDevice({ query, appCheckToken: token });
    if (!res.ok) {
      setPhase("error");
      setError(res.error.message);
      return;
    }
    setDraft(res.data.device);
    setRaw(res.data.raw);
    setSaved(null);
    setPhase("done");
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-semibold tracking-tight">Device Editor</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            AI Magic Auto-Fill retrieves, structures, and verifies every spec — you review before
            publishing.
          </p>
        </div>
        {draft ? (
      ...
      {saved ? (
        <div className="flex items-center gap-3 rounded-xl border border-success/40 bg-success/10 p-4 text-sm">
          ...
          <span>
            ...
            .{" "}
            {saved.published
              ? "Published — it is now public in the catalog."
              : "Draft saved. An admin can promote it through the status pipeline."}
          </span>
      ...
      {!draft ? (
        <Card className="relative overflow-hidden">
          <CardHeader>
            ...
            <CardDescription>
              Describe a device — &quot;Samsung Galaxy S25 Ultra&quot; — and Gemini returns a
              100%-coverage spec sheet with per-field confidence ratings.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex gap-2">
              <Input
                ...
                placeholder='e.g. "OnePlus 13R", "Xiaomi 16 Pro", "iPhone 17 Air"'
                className="h-11"
                disabled={phase === "extracting"}
              />
              <Button size="lg" onClick={onAutofill} disabled={phase === "extracting" || query.trim().length < 3}>
                {phase === "extracting" ? <Loader2 className="size-4 animate-spin" /> : <Bot className="size-4" />}
                {phase === "extracting" ? "Extracting…" : "Auto-Fill"}
              </Button>
            </div>
            {phase === "extracting" ? (
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Retrieving sources, structuring specs, and rating confidence…
                </p>
              </div>
            ) : null}
            <div className="flex items-center gap-4 text-xs text-muted-foreground">
              <span className="inline-flex items-center gap-1.5">
                <Badge variant="success">Verified</Badge> official / cross-checked
              </span>
              <span className="inline-flex items-center gap-1.5">
                <Badge variant="warning">Estimated</Badge> inferred
              </span>
              <button onClick={onStartBlank} className="ml-auto font-medium text-primary hover:underline">
        ...
        <>
          ...
          <Card>
            <CardContent className="grid gap-4 py-4 sm:grid-cols-4">
              <div>
                <div className="text-xs text-muted-foreground">Coverage confidence</div>
                <div className="mt-1 flex items-center gap-2">
                  <Progress value={Math.round(draft.confidence.overall * 100)} className="h-2 flex-1" />
              ...
              <div className="text-sm">
                ...
                <div className="font-medium text-success">{draft.confidence.verifiedFields.length} fields</div>
              </div>
              <div className="text-sm">
                <div className="text-xs text-muted-foreground">Estimated</div>
                <div className="font-medium text-warning">{draft.confidence.estimatedFields.length} fields</div>
              </div>
              <div className="text-sm">
                <div className="text-xs text-muted-foreground">Unavailable</div>
                <div className="font-medium text-muted-foreground">{draft.confidence.unavailableFields.length} fields</div>
              </div>
            </CardContent>
          </Card>
          ...
          <Tabs defaultValue="structured">
            <TabsList>
              ...
              <TabsTrigger value="raw">
                AI data vs raw source JSON
                {raw ? <span className="ml-1.5 rounded-full bg-primary/15 px-1.5 text-[10px]">{JSON.parse(raw).sources?.length ?? 0}</span> : null}
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="raw" className="mt-4 space-y-6">
              {raw ? (
                <>
                  <Card>
                    <CardHeader>
                      <CardTitle className="text-base">AI data vs raw source JSON</CardTitle>
                      <CardDescription>
                        Left: the structured draft you&apos;re about to save. Right: the raw model
                        response. Review fields the model flagged as estimated or unavailable.
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 lg:grid-cols-2">
                      <div>
                        <div className="mb-1.5 text-xs font-medium text-muted-foreground">Structured draft</div>
                      ...
                      <div>
                        <div className="mb-1.5 text-xs font-medium text-muted-foreground">Raw model JSON</div>
                        <pre className="max-h-96 overflow-auto rounded-lg border border-border bg-muted/40 p-3 font-mono text-[11px] leading-relaxed">
                  ...
                  <Card>
                    <CardHeader>
                      <button onClick={() => setSourcesOpen((o) => !o)} className="flex w-full items-center justify-between text-left">
                        <CardTitle className="text-base">Sources ({draft.sources.length})</CardTitle>
                        <Badge variant="outline">{sourcesOpen ? "Hide" : "Show"}</Badge>
                      </button>
                    </CardHeader>
                    {sourcesOpen ? (
                      <CardContent>
                        <ul className="space-y-2">
                          {draft.sources.map((s, i) => (
                            <li key={i} className="flex items-center gap-3 text-sm">
                              ...
                              <a href={s.url} target="_blank" rel="noreferrer" className="truncate text-primary underline">
                                {s.title}
                              </a>
                            </li>
                          ))}
                ...
                <p className="rounded-xl border border-border p-8 text-center text-sm text-muted-foreground">
                  No AI extraction yet — this panel appears after running Magic Auto-Fill.
                </p>
              )}
            </TabsContent>
          </Tabs>
